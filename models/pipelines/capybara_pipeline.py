...
from models.models.autoencoders import hunyuanvideo_15_vae
from models.models.text_encoders import PROMPT_TEMPLATE
from models.models.text_encoders import TextEncoder
from models.models.text_encoders.byT5 import load_glyph_byT5_v2
from models.models.text_encoders.byT5.format_prompt import MultilingualPromptFormat
from models.models.transformers.hunyuanvideo_1_5_transformer_capybara import (
    HunyuanVideo_1_5_DiffusionTransformer,
)
from models.models.transformers.modules.upsample import SRTo720pUpsampler, SRTo1080pUpsampler
from models.models.vision_encoder import VisionEncoder

from models.schedulers.scheduling_flow_match_discrete import FlowMatchDiscreteScheduler
...
from models.utils.multitask_utils import (
...
def load_video_frames(video_path, target_fps=24, max_frames=81, target_size=(480, 848)):
    ...
    Returns:
        tuple: (torch.Tensor, numpy.ndarray)
            ...
            - numpy.ndarray: Frame as numpy array of shape (T, H, W, 3) with values in [0, 255]
    """
    try:
        ...
        # Get all frames as numpy array (T, H, W, 3) for vision encoder
        # Convert from (T, C, H, W) to (T, H, W, C)
        # Values are still in [0, 255] range at this point
        frame_tensor = frames.permute(0, 2, 3, 1)  # (T, H, W, C)
        # Convert to numpy and ensure uint8 format
        frame_numpy = frame_tensor.clamp(0, 255).cpu().numpy().astype(np.uint8)
...
def load_video_key_frames(video_path, frames_numpy=None, max_frames=81, target_size=(480, 848)):
    """
    Load key frames (first, middle, last) from video using decord
    
    Args:
        video_path (str): Path to the video file
        frames_numpy (numpy.ndarray): Frame as numpy array of shape (T, H, W, 3) with values in [0, 255]
        max_frames (int): Maximum number of frames to extract
        target_size (tuple): Target size (height, width) for resizing
    ...
    # If given frames_numpy, use it to load key frames
    if frames_numpy is not None:
        total_frames = frames_numpy.shape[0]
        key_indices = [0, total_frames // 2, total_frames - 1]
        key_frames_numpy = frames_numpy[key_indices]

        # transform to torch tensor
        key_frames_tensor = torch.from_numpy(key_frames_numpy).float()
        key_frames_tensor = key_frames_tensor.permute(0, 3, 1, 2)
        key_frames_tensor = key_frames_tensor / 127.5 - 1.0
        key_frames_tensor = key_frames_tensor.unsqueeze(0)
        key_frames_tensor = key_frames_tensor.permute(0, 2, 1, 3, 4)

        return key_frames_tensor, key_frames_numpy

    # If not given frames_numpy, load key frames from video
    try:
        # Initialize video reader
        vr = decord.VideoReader(video_path, ctx=decord.cpu(0))
        total_frames = min(max_frames, len(vr))

        assert len(vr) >= max_frames, f"Video {video_path} has only {len(vr)} frames, but max_frames is set to {max_frames}"
        ...
        if total_frames == 1:
            ...
            if 81 < len(vr) < 121:
                start_idx = len(vr) - max_frames
            else:
                start_idx = 0
            key_indices = [start_idx, total_frames // 2 + start_idx, total_frames - 1 + start_idx]
        ...
        logging.error(f"Error loading key frames from video {video_path}: {str(e)}")
        return None, None


@dataclass
class CapybaraPipelineOutput(BaseOutput):
    videos: Union[torch.Tensor, np.ndarray]
    sr_videos: Optional[Union[torch.Tensor, np.ndarray]] = None


class Capybara_Pipeline(DiffusionPipeline):
    ...
    def _prepare_vision_states(self, task_type,reference_vision, height, width, latents, device):
        """
        Prepare vision states for multitask training.

        Args:
            reference_vision: Reference vision for i2v tasks (None for t2v tasks).
        ...
        if reference_vision is None:
            ...
            if task_type in ["i2v", "ti2i"]:
                reference_vision = np.array(reference_vision) if isinstance(reference_vision, Image.Image) else reference_vision
                if len(reference_vision.shape) == 4:
                    reference_vision = reference_vision[0]

                # Encode reference image to vision states
                if self.vision_encoder is not None:
                    input_image_np = resize_and_center_crop(reference_vision, target_width=width, target_height=height)
                    ...
                    vision_states = None
            elif task_type in ["tv2v", "prop", "dense_prediction", "cond_generation", "style_transfer", "iv2v", "vv2v"]:
                vision_states_list = []
                for i in range(reference_vision.shape[0]):
                    frame = reference_vision[i]
                    frame_states = self.vision_encoder.encode_images(frame)
                    vision_states_list.append(frame_states.last_hidden_state)
                vision_states = torch.cat(vision_states_list, dim=0).unsqueeze(0)
            else:
                vision_states = None
    ...
    def _prepare_cond_latents(self, task_type, cond_latents, latents, multitask_mask):
        ...
        if cond_latents is not None and task_type in ['i2v']:
            latents_concat = cond_latents.repeat(1, 1, latents.shape[2], 1, 1)
            latents_concat[:, :, 1:, :, :] = 0.0
        elif cond_latents is not None and task_type in ['tv2v', 'ti2i']:
    ...
    def get_task_mask(self, task_type, latent_target_length):
        ...
        elif task_type in ["i2v"]:
            mask = torch.zeros(latent_target_length)
            mask[0] = 1.0
        elif task_type in ["tv2v", "ti2i"]:
            mask = torch.ones(latent_target_length)
        else:
    ...
    def get_channel_condition_latents(self, task_type, reference_vision, height, width):

        if task_type in ["t2v", "t2i"]:
            cond_latents = None

        elif task_type in ["i2v", "ti2i"]:
            origin_size = reference_vision.size
            ...
            ref_images_pixel_values = ref_image_transform(reference_vision).unsqueeze(0).unsqueeze(2).to(self.execution_device)
            ...
            reference_vision = reference_vision.to(self.execution_device)
            with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=True):
                self.vae.enable_tiling()
                cond_latents = self.vae.encode(reference_vision).latent_dist.mode()
                cond_latents.mul_(self.vae.config.scaling_factor)
                self.vae.disable_tiling()
            
        else:
            raise ValueError(f"Unsupported task_type: {task_type}. Must be in ['t2v', 'i2v', 'tv2v']")
        ...
        **kwargs,
        ...
        Args:
            ...
            return_dict (`bool`, *optional*, defaults to True):
                Whether to return a [`CapybaraPipelineOutput`] or a tuple.
            **kwargs:
                Additional keyword arguments.

        Returns:
            CapybaraPipelineOutput or `tuple`:
                If `return_dict` is True, returns a [`CapybaraPipelineOutput`] with fields:
        ...
        if task_type == "tv2v":
            assert "reference_video" in kwargs, "reference_video is required for tv2v tasks"
            reference_video_path = kwargs.get("reference_video")
            reference_video, reference_video_numpy = load_video_frames(
                reference_video_path,
                target_fps=24,
                max_frames=video_length,
                target_size=(height, width),
            )
            key_frames_tensor, key_frames_numpy = load_video_key_frames(
                reference_video_path,
                frames_numpy=reference_video_numpy,
                max_frames=video_length,
                target_size=(height, width),
            )
        with auto_offload_model(self.vae, self.execution_device, enabled=self.enable_offloading):
            if task_type == "tv2v":
                channel_cond = self.get_channel_condition_latents(task_type, reference_video, height, width)
            elif task_type in ["ti2i", "i2v"]:
                assert "reference_img" in kwargs, "reference_img is required for ti2i tasks"
                reference_img_path = kwargs.get("reference_img")
                reference_img = Image.open(reference_img_path).convert('RGB')
                reference_img_np = np.array(reference_img)
                channel_cond = self.get_channel_condition_latents(task_type, reference_img, height, width)
            else:
                channel_cond = self.get_channel_condition_latents(task_type, reference_image, height, width)

        cond_latents = self._prepare_cond_latents(
            task_type, channel_cond, latents, multitask_mask
        )
        ...
        with auto_offload_model(self.vision_encoder, self.execution_device, enabled=self.enable_offloading):
            if task_type == "tv2v":
                vision_states = self._prepare_vision_states(
                    task_type, key_frames_numpy, height, width, latents, device
                )
            elif task_type == "ti2i":
                vision_states = self._prepare_vision_states(
                    task_type, reference_img_np, height, width, latents, device
                ).unsqueeze(0)
            else:
                vision_states = torch.zeros(1, 1, 729, 1152).to(self.execution_device, dtype=self.target_dtype)
                if self.do_classifier_free_guidance:
                    vision_states = vision_states.repeat(2, 1, 1, 1)
        ...
        return CapybaraPipelineOutput(videos=video_frames)
