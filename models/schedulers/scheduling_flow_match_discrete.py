...
class FlowMatchDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step_ic(
        ...
        timestep: Union[float, torch.FloatTensor],
        sample1: torch.FloatTensor,
        sample2: torch.FloatTensor,
        generator: Optional[torch.Generator] = None,
        n_tokens: Optional[int] = None,
        return_dict: bool = True,
    ) -> Union[FlowMatchDiscreteSchedulerOutput, Tuple]:
        ...
        # Upcast to avoid precision issues when computing prev_sample
        sample1 = sample1.to(torch.float32)
        sample2 = sample2.to(torch.float32)

        dt = self.sigmas[self.step_index + 1] - self.sigmas[self.step_index]

        if self.config.solver == "euler":
            prev_sample1 = sample1 + model_output1.float() * dt
            prev_sample2 = sample2 + model_output2.float() * dt
        else:
            raise ValueError(f"Solver {self.config.solver} not supported. Supported solvers: {self.supported_solver}")
        ...
        if not return_dict:
            return (prev_sample1, prev_sample2)

        return FlowMatchDiscreteSchedulerOutput(prev_sample1=prev_sample1, prev_sample2=prev_sample2)
