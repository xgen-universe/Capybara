...
def load_text_encoder(
    ...
    # NOTE:
    # - For pure text LLM encoders, AutoModel works and returns last_hidden_state.
    # - For Qwen2.5-VL style multimodal encoders, we need the VL conditional generation class
    #   to accept vision inputs (pixel_values/image_grid_thw, etc.). We detect this via config.
    model_cls = None
    try:
        cfg = AutoConfig.from_pretrained(text_encoder_path)
        arch = (getattr(cfg, "architectures", None) or [None])[0]
        if arch and "Qwen2_5_VLForConditionalGeneration" in arch:
            from transformers import Qwen2_5_VLForConditionalGeneration  # type: ignore
            model_cls = Qwen2_5_VLForConditionalGeneration
    except Exception:
        # Best-effort detection; fallback to AutoModel.
        model_cls = None

    is_vl_model = model_cls is not None
    if is_vl_model:
    ...
    # IMPORTANT:
    # Some multimodal wrappers (e.g. Qwen2.5-VL) expose `language_model`, but we MUST keep the full
    # model to accept vision inputs. Only unwrap for pure-text backbones.
    if (not is_vl_model) and hasattr(text_encoder, "language_model"):
        text_encoder = text_encoder.language_model
    # Some LLM backbones expose `norm` as final layer norm (e.g. Qwen-style).
    if hasattr(text_encoder, "norm") and not hasattr(text_encoder, "final_layer_norm"):
...
def load_tokenizer(
    ...
    # Optional: for multimodal models (e.g. Qwen2.5-VL), we also need AutoProcessor.
    try:
        min_pixels = 256*28*28
        max_pixels = 512*28*28
        processor = AutoProcessor.from_pretrained(tokenizer_path, min_pixels=min_pixels, max_pixels=max_pixels)
    except Exception:
        processor = None
...
class TextEncoder(nn.Module):
    ...
    def encode(
        ...
        # Support both:
        # - pure text tokenization outputs (dict with input_ids/attention_mask)
        # - multimodal processor outputs (may include pixel_values, image_grid_thw, etc.)
        model_inputs = dict(batch_encoding)

        attention_mask = None
        if use_attention_mask and "attention_mask" in model_inputs:
            # Keep a copy for returning / cropping logic.
            attention_mask = model_inputs["attention_mask"]
            if isinstance(attention_mask, torch.Tensor):
                attention_mask = attention_mask.to(device)

        if not use_attention_mask and "attention_mask" in model_inputs:
        ...
        # Ensure hidden states are available when needed.
        need_hidden_states = output_hidden_states or hidden_state_skip_layer is not None
