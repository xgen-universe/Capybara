...
from models.utils.communications import all_gather
from models.utils.infer_utils import torch_compile_wrapper
from models.models.text_encoders.byT5 import ByT5Mapper
...
class MMDoubleStreamBlock(nn.Module):
    ...
    def forward(
        ...
        siglip_freqs_cis: tuple = None,
        siglip_token_len: Union[int, Tuple[int, int]] = None,
        text_mask=None,
        attn_param=None,
        is_flash=False,
        block_idx=None,
        ...
        # 如果 siglip_token_len 不为 None，将 txt 分为 siglip tokens 和普通 txt 两部分，对 siglip tokens 的 q k 单独做 RoPE
        if siglip_token_len is not None and siglip_freqs_cis is not None:
            # 处理 siglip_token_len 是 int 或 tuple 两种情况
            if isinstance(siglip_token_len, int):
                # int 情况：从开头到 siglip_token_len 的 tokens 需要加 RoPE
                start_idx = 0
                end_idx = siglip_token_len
            elif isinstance(siglip_token_len, tuple) and len(siglip_token_len) == 2:
                # tuple 情况：(start_idx, end_idx) 指定需要加 RoPE 的范围
                start_idx, end_idx = siglip_token_len
            else:
                raise ValueError(f"siglip_token_len must be int or tuple of 2 ints, got {type(siglip_token_len)}")
            
            if end_idx > start_idx and end_idx <= txt_q.shape[1]:
                siglip_txt_q = txt_q[:, start_idx:end_idx, :, :]
                siglip_txt_k = txt_k[:, start_idx:end_idx, :, :]
                
                # 对 siglip tokens 的 q k 使用 siglip_freqs_cis 做 RoPE
                siglip_txt_qq, siglip_txt_kk = apply_rotary_emb(siglip_txt_q, siglip_txt_k, siglip_freqs_cis, head_first=False)
                assert (
                    siglip_txt_qq.shape == siglip_txt_q.shape and siglip_txt_kk.shape == siglip_txt_k.shape
                ), f"siglip_txt_qq: {siglip_txt_qq.shape}, siglip_txt_q: {siglip_txt_q.shape}, siglip_txt_kk: {siglip_txt_kk.shape}, siglip_txt_k: {siglip_txt_k.shape}"
                
                # 重新拼接 siglip tokens 和普通 txt 的 q k
                if start_idx == 0:
                    # siglip tokens 在开头
                    normal_txt_q = txt_q[:, end_idx:, :, :]
                    normal_txt_k = txt_k[:, end_idx:, :, :]
                    txt_q = torch.cat([siglip_txt_qq, normal_txt_q], dim=1)
                    txt_k = torch.cat([siglip_txt_kk, normal_txt_k], dim=1)
                else:
                    # siglip tokens 在中间或末尾
                    before_txt_q = txt_q[:, :start_idx, :, :]
                    before_txt_k = txt_k[:, :start_idx, :, :]
                    after_txt_q = txt_q[:, end_idx:, :, :]
                    after_txt_k = txt_k[:, end_idx:, :, :]
                    txt_q = torch.cat([before_txt_q, siglip_txt_qq, after_txt_q], dim=1)
                    txt_k = torch.cat([before_txt_k, siglip_txt_kk, after_txt_k], dim=1)
...
class HunyuanVideo_1_5_DiffusionTransformer(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def __init__(
        ...
        self.cond_special_token_embedding = nn.Embedding(4, self.hidden_size)
        self.cond_special_token_embedding.weight.data.fill_(0)

        self.siglip_type_embedding = nn.Embedding(2, self.hidden_size)
        self.siglip_type_embedding.weight.data.fill_(0)

        self.task_bias = nn.Embedding(10, self.hidden_size)
        self.task_bias.weight.data.fill_(0)
    ...
    def get_rotary_pos_embed(self, rope_sizes, ref_rope=False, ref_rope_sizes=None):
        target_ndim = 3
    ...
    def forward(
        ...
        # before img_in, img (B, C, T, H, W)
        img = self.img_in(img)
        # after img_in, img (B, T*H*W, 2048)
        ...
        if self.vision_in is not None and vision_states is not None:
            ...
            siglip_f = extra_encoder_hidden_states.shape[1]
            siglip_n = extra_encoder_hidden_states.shape[2]
            extra_encoder_hidden_states = rearrange(extra_encoder_hidden_states, 'b f n c-> b (f n) c')
            # If t2v, set extra_attention_mask to 0 to avoid attention to semantic tokens
            if mask_type in ["t2v", "s2v", "t2i"] and torch.all(vision_states == 0):
            ...
            if siglip_f > 1 and mask_type not in ["t2v", "t2i", "i2v"]:
                siglip_freqs_cos, siglip_freqs_sin = self.get_rotary_pos_embed(
                    None, ref_rope=True, ref_rope_sizes=((0, 0, 0), (siglip_f, 1, 1))
                )  # (F, head_dim)
                siglip_freqs_cos = siglip_freqs_cos.repeat_interleave(siglip_n, dim=0)  # (F*N, head_dim)
                siglip_freqs_sin = siglip_freqs_sin.repeat_interleave(siglip_n, dim=0)
                siglip_token_len = siglip_f * siglip_n
            else:
                siglip_freqs_cos = None
                siglip_freqs_sin = None
                siglip_token_len = None

            # Ensure valid tokens precede padding tokens
            if self.cond_type_embedding is not None:
                if mask_type in ["t2v", "t2i", "i2v"]:
                    cond_emb = self.cond_type_embedding(
                        2 * torch.ones_like(
                    ...
                    cond_emb = self.siglip_type_embedding(
                        1 * torch.ones_like(
                            extra_encoder_hidden_states[:, :, 0],
                            dtype=torch.long,
                            device=extra_encoder_hidden_states.device,
        ...
        if siglip_freqs_cos is not None:
            siglip_freqs_cis = (siglip_freqs_cos, siglip_freqs_sin)
        else:
            siglip_freqs_cis = None

        def custom_forward(block):
            def fwd(img, txt, vec, freqs_cis, siglip_freqs_cis, siglip_token_len, text_mask):
                return block(
                    img=img,
                    txt=txt,
                    vec=vec,
                    freqs_cis=freqs_cis,
                    siglip_freqs_cis=siglip_freqs_cis,
                    siglip_token_len=siglip_token_len,
                    text_mask=text_mask,
